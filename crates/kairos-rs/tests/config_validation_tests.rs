...
fn test_empty_configuration() {
    ...
    assert!(result
        .errors
        .iter()
        .any(|e| e.contains("No routers configured")));
}
...
fn test_security_warnings() {
    let settings = Settings {
        ...
        ai: None,
        version: 1,
        routers: vec![create_test_router(
            "http://example.com",
            "/api/test",
            vec!["GET"],
        )],
    ...
    assert!(result
        .warnings
        .iter()
        .any(|w| w.contains("Insecure HTTP backend")));
}
...
fn test_performance_warnings() {
    ...
    assert!(result
        .warnings
        .iter()
        .any(|w| w.contains("many parameters")));
}
...
fn test_overly_permissive_methods() {
    ...
    assert!(result
        .warnings
        .iter()
        .any(|w| w.contains("many HTTP methods")));
}
...
fn test_duplicate_route_paths() {
    ...
    assert!(result
        .errors
        .iter()
        .any(|e| e.contains("Duplicate route path")));
}
...
fn test_mixed_http_https_warnings() {
    ...
    assert!(!result
        .warnings
        .iter()
        .any(|w| w.contains("All routes use HTTP")));
}
...
fn test_high_dynamic_routes_warning() {
    ...
    for i in 0..60 {
        routers.push(create_test_router(
            "https://example.com",
            &format!("/api/{{id{}}}", i),
            vec!["GET"],
        ));
    ...
    assert!(result
        .warnings
        .iter()
        .any(|w| w.contains("High number of dynamic routes")));
}
...
fn test_static_route_recommendation() {
    ...
    assert!(result
        .recommendations
        .iter()
        .any(|r| r.contains("static routes")));
}
